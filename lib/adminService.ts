...
export class IndexedDBAdminService {
  ...
  validateRecord(storeName: string, record: any): { valid: boolean; errors: string[] } {
    ...
    switch (storeName) {
      ...
      case "periodRecords":
        ...
        if (!record.dateTime) {
          errors.push("必须包含日期时间字段")
        }
        if (!record.status || !["start", "ongoing", "end"].includes(record.status)) {
          errors.push("状态字段必须是有效值")
        }
        if (!record.flow || !["minimal", "light", "normal", "heavy"].includes(record.flow)) {
          errors.push("流量字段必须是有效值")
        }
        break
    }
  ...
  async ensureIDB() {
    ...
    if (
      !(
        currentStores.includes("users") &&
        currentStores.includes("stoolRecords") &&
        currentStores.includes("myRecords") &&
        currentStores.includes("mealRecords") &&
        currentStores.includes("periodRecords")
      )
    ) {
