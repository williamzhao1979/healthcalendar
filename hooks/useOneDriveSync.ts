...
import { dataExportService, type ExportResult } from "../lib/dataExportService"
...
export const useOneDriveSync = (): [OneDriveSyncState, OneDriveSyncActions] => {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // 使用移动端友好的错误信息
      const friendlyError =
        error instanceof Error ? MobileCompatibilityUtils.getUserFriendlyErrorMessage(error) : "连接失败"
    ...
    async (userId: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // 部分成功或完全失败
          const errorMessage = result.errors.length > 0 ? result.errors.join("; ") : "同步过程中出现未知错误"
          ...
          console.warn(
            `Sync completed with errors. Imported: ${result.importedTables.join(", ")}, Errors: ${result.errors.join(", ")}`,
          )
        }
      } catch (error) {
  ...
  const loadFileContent = useCallback(
    async (fileId: string, fileName: string, isFolder = false) => {
      if (!state.isAuthenticated) {
        setState((prev) => ({ ...prev, error: "未连接到OneDrive" }))
        return
      }
  ...
  const syncIDBOneDrivePeriodRecords = useCallback(async () => {
    ...
    try {
      const graphClient = microsoftAuth.getGraphClient()!
      console.log("Starting period records import from OneDrive...")
      ...
      setState((prev) => ({
        ...
        exportResult: result.success
          ? {
              success: true,
              exportedFiles: [`已导入 ${result.importedCount} 个生理记录`],
              errors: result.errors,
              metadata: {
                version: "1.0",
                exportTime: new Date().toISOString(),
