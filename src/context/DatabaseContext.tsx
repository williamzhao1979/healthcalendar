...
export function DatabaseProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const createDefaultUser = async () => {
      // 只有当数据库已初始化，且用户列表已加载完成且为空时才创建默认用户
      if (dbInitialized && !usersLoading && users.length === 0 && createUserAttempts < MAX_CREATE_USER_ATTEMPTS) {
        try {
          ...
          // 直接使用数据库服务，绕过hook层
          const newUser = await dbService.addUser({ name: "我" });
          console.log("默认用户创建成功:", newUser);
          
          // 直接更新设置，绕过hook层
          await dbService.updateSettings({ lastUserId: newUser.id });
          console.log("默认用户设置为当前用户");
          
          // 手动触发用户列表重新加载
          setCreateUserAttempts(MAX_CREATE_USER_ATTEMPTS); // 停止重试
